...
exports.default = {
    ...
    exportExcel: function exportExcel(json) {
        if (json["links"].length) {
            ...
            convertData.forEach(function (d, i) {
				...
				convertDataExtract[i]["nodeId"] = d["target"]["id"];
				convertDataExtract[i]["parentNodeId"] = d["source"]["id"];
                convertDataExtract[i]["nodeName"] = d["target"]["group"];
                convertDataExtract[i]["nodeNote"] = d["target"]["nodeNote"];
				convertDataExtract[i]["nodeTitle"] = d["target"]["nodeTitle"];
            });
	...
	//停止模拟按钮
	stoporigin:function stoporigin(json, update, vis, force, node, link) {
        force.stop();
    },
	//启动
	resumerigin:function resumerigin(json, update, vis, force, node, link) {
        force.restart();
    },
	//还原
	backrigin:function backrigin(json, update, vis, force, node, link) {
...
var link = svg.selectAll(".link")
	...
	.attr("r", 5)
	.attr("stroke","black")
	.attr("stroke-width",1).call( d3.drag().on("drag", dragended));
  ...
  force.on("tick", function() {
	...
	//拖动tuodongrigin
	//启动
	tuodongrigin:function tuodongrigin(json, update, vis, force, node, link) {
		//阻止父级元素
	  ...
	  //临时记录被拖放的元素
	   ...
	   //开始拖放事件监听
	    // vis.addEventListener('dragstart', function(e) {
			
              //保存当前拖放对象
            //    elementDragged = this;
           // });
            //结束拖放事件监听
         //  vis.addEventListener('dragend', function(e) {
                //注销当前拖放对象
           //     elementDragged = null;
          //  });
    },
...
exports.default = function (json, update, vis, force, node, link) {
	...
	//取消选中事件J_clecckrigin
	 d3.select('#J_clecckrigin').on('click',function(){
		    d3.selectAll('.node.active').classed('active', false);
	 })
	 //为选中节点批量添加下级节点
	  d3.select('#J_pliangrigin').on('click',function(){
		 arry = d3.selectAll('.node.active').data();
		 if(arry.length==0){
			  alert("请选则要添加的节点");
			  return;
		 }
   ...
   //批量添加节点
      d3.select('#J_PAddNode').on("click.add-node", function () {
		$('.list-input').val('');
		$('#addOrUpdate').val("2");
		$("#one").css('display','block'); 
		$("#two").css('display','block'); 
	...
	//修改节点数据  添加'0'==addOrUpdate 修改节点 '1'==addOrUpdate 批量添加节点无连线 '2'==addOrUpdate  4批量添加下级节点有连线，5批量添加选中节点的下级节点
  d3.select('#login-button-submit').on('click',function(){
			...
			var nodeNote =$("#jdSelect").val();
			var nodeTitle= $("#xjdSelectType").val();
            //var newNodeId = +new Date(); 需要填充节点id,节点名称,下级节点标题,节点注释 四个选项
            var newNode = { "id": newNodeId, "group": group,"nodeNote":nodeNote,"nodeTitle":nodeTitle,isNew: true };
           ...
           // d3.select("g#node-" + newNodeId).attr("transform", "translate(" + tx + "," + ty + ")").datum(Object.assign(d3.select("g#node-" + newNodeId).data()[0], { "x": tx, "y": ty }));
		   }else if('1'==addOrUpdate){
				 //修改节点
				 //修改节点时，要把标签内的id同时替换。取消选中节点时用到
				var  uuid= getUuid();
				//获取节点
				d3.select("#node-" + selNodes[0]["id"]).attr("id","node-"+uuid);
				 //
				  selNodes[0]["id"]=uuid;
                  selNodes[0].group=$("#name").val();
                  selNodes[0]["nodeNote"]=$("#jdSelect").val();
                  selNodes[0]["nodeTitle"]=$("#xjdSelectType").val();
			      updates(jsones);   
				 //添加下级节点连线
			   }else if('3'==addOrUpdate){
				   //获取下级节点数据
				 var _util$getTranslateAnd = _util2.default.getTranslateAndScale(),
                translate = _util$getTranslateAnd.translate,
                scale = _util$getTranslateAnd.scale;
            ...
            //var newNodeId = +new Date(); 需要填充节点id,节点名称,下级节点标题,节点注释 四个选项
            var newNode = { "id": newNodeId, "group": group,"nodeNote":nodeNote,"nodeTitle":nodeTitle,isNew: true };
            addjson.nodes.push(newNode);
			var target=null;
			var source=null;
			for(var i=0;i<addjson.nodes.length;i++){
				if(newNodeId==addjson.nodes[i].id){
					target=addjson.nodes[i];
				}
				if(oldId==addjson.nodes[i].id){
					source=addjson.nodes[i];
				}
			...
			var _link = { source: source, target: target };
			addjson.links.push(_link);
            addupdate(addjson);
           // var tx = x / scale - +translate[0] / scale,
           //     ty = y / scale - +translate[1] / scale;
           // d3.select("g#node-" + newNodeId).attr("transform", "translate(" + tx + "," + ty + ")").datum(Object.assign(d3.select("g#node-" + newNodeId).data()[0], { "x": tx, "y": ty }));		   
            }else if('4'==addOrUpdate){
			  //批量添加连线
	           var plname = $("#name").val();
			   var number;
				if(""==plname){
					number=　parseInt($("#pnumbe").val());
					if(number>100){
				   alert("请分批次添加，以免浏览器卡死，添加100节点大约18秒，若在此基础上在添加100节点，时间将远大于18秒，请自测。");	
				   return;
				   }
			 for(var i=0;i<number;i++){
                ...
                y = _d3$mouse2[1];
		      var newNodeId =getUuid();;
              var newNode = { "id":newNodeId , "group": "","nodeNote":"","nodeTitle":"",isNew: true };
              addjson.nodes.push(newNode);
			 var target=null;
			 var source=null;
			for(var j=0;j<addjson.nodes.length;j++){
				if(newNodeId==addjson.nodes[j].id){
					target=addjson.nodes[j];
				}
				if(oldId==addjson.nodes[j].id){
					source=addjson.nodes[j];
				}
        ...
        //    d3.select("g#node-" + newNodeId).attr("transform", "translate(" + tx + "," + ty + ")").datum(Object.assign(d3.select("g#node-" + newNodeId).data()[0], { "x": tx, "y": ty }));		   
			...
			for(var j=0;j<json.nodes.length;j++){
				if(newNodeId==json.nodes[j].id){
					target=json.nodes[j];
				}
				if(arry[k].id==json.nodes[j].id){
					source=json.nodes[j];
				}
	...
	//停
	 d3.select('#J_stoporigin').on("click.change-layout", function () {
        _util2.default.stoporigin(json, update, vis, force, node, link);//json, update, vis, force, node, link
    });
	//放
	 d3.select('#J_resumerigin').on("click.change-layout", function () {
        _util2.default.resumerigin(json, update, vis, force, node, link);
    });
	//还原
	d3.select('#J_BackLayout').on("click.change-layout", function () {
        _util2.default.backrigin(json, update, vis, force, node, link);
    });
	//拖动
	d3.select('#J_tuodong').on("click.change-layout", function () {
        _util2.default.tuodongrigin(json, update, vis, force, node, link);
    });
};

exports.__delNode = __delNode;
exports.__delLink = __delLink;
exports._updatenode=_updatenode;
exports._addnodelink=_addnodelink;
exports._addnodelinks=_addnodelinks;
exports._quxiaoselect=_quxiaoselect;
...
//添加下级节点连线
function _addnodelink(json, update, selNode){
	$('.list-input').val('');
	 addjson=json;
	 addupdate=update;
	 oldId=selNode[0]["id"];
...
//批量添加下级节点连线
function _addnodelinks(json, update, selNode){
	$('.list-input').val('');
	 addjson=json;
	 addupdate=update;
	 oldId=selNode[0]["id"];
...
//修改节点
function _updatenode(json, update, selNode) {
   selNode = selNode || d3.selectAll('.node.active').data();
   $('.list-input').val('');
   $("#name").val(selNode[0].id);
   $("#jdSelectType").val(selNode[0]["id"]);
   $("#xjdSelectType").val(selNode[0]["nodeTitle"]);
   $("#jdSelect").val(selNode[0]["nodeNote"]);
...
//取消当前节点选中事件
function _quxiaoselect(selNode){
	var id=selNode[0].id;
	id="node-"+id;
    d3.selectAll('#'+id).classed('active', false);	
	console.info(d3.selectAll('#'+id));
	console.info(d3.selectAll('.node.active'));
}
...
exports.default = function () {
    ...
    return d3.forceSimulation([]).force("link", d3.forceLink([]).id(function (d) {
        return d.id;
    }).distance(120)).force("charge", d3.forceManyBody().strength(-2500)).force("center", d3.forceCenter($('.graph-area').width() / 2, $('.graph-area').height() / 2)).force("x", d3.forceX()).force("y", d3.forceY()).force("collide", d3.forceCollide().strength(0.2).iterations(5));
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0), __webpack_require__(1)))
...
exports.default = function (json, vis) {
    ...
    }).attr("stroke", "#130504").attr("stroke-width", 0.9).attr("fill", "none");
    return link;
};
...
exports.default = function (json, vis) {
	...
	//
	//var force = vis.selectAll("g.force");
    //
   ...
   //node.append('svg:circle').attr("class", "node-bg").attr("r", "12px").attr("fill", "#5bc0de");
	//绘制一个圆
	node.append('svg:circle').attr("class", "node-bg").attr("r", "14px").attr("fill", "#3b1900");
    node.append("svg:image").attr("class", "node-icon").attr("xlink:href", _node2.default).attr("x", "-13px").attr("y", "-12px").attr("width", "26px").attr("height", "26px");
    ...
    node.append("svg:text").attr("class", "node-group").attr("dy", function (d) {
        return d["id"] ? "21px" : "21px";
    }).attr("text-anchor", "middle").attr("font-size", "15px").attr("fill", "#060000").html(function (d) {
        return d["group"];
    });
...
exports.default = function (json, update, vis, node, link) {
    ...
    node.on("contextmenu", function (d) {
        d.contextmenuData = {
            "title": "操作当前节点",
             "list": [{ "class": "node-del", "text": "删除节点" },{ "class": "update-node", "text": "修改节点" },{ "class": "add-nodelink", "text": "添加下级节点" },{ "class": "add-nodelinks", "text": "批量添加下级节点" },{ "class": "add-quxiaoselect", "text": "取消当前节点选中" }]
        };
        ...
        d3.select(".node-del").on("click", function () {
		 ...
		 //绑定右键修改节点数据
		  d3.select(".update-node").on("click", function () {
            (0, _bindEvent._updatenode)(json, update, [d]);
			setTimeout(function () {
                
            });
        });
		//添加下级节点
		 d3.select(".add-nodelink").on("click", function () {
            (0, _bindEvent._addnodelink)(json, update, [d]);
			setTimeout(function () {
                
            });
        });
		//批量添加
		 d3.select(".add-nodelinks").on("click", function () {
            (0, _bindEvent._addnodelinks)(json, update, [d]);
			setTimeout(function () {
                
            });
        });
		//取消选中事件
		 d3.select(".add-quxiaoselect").on("click", function () {
            (0, _bindEvent._quxiaoselect)([d]);
			setTimeout(function () {
                  ct.showContextMenu(null);
            });
        });
...
function graphInit(json) {
    ...
    function update() {
        ...
        var bindLinkAndNodeEvent = (0, _bindLinkAndNodeEvent3.default)(json, update, vis, node, link);//鼠标右击事件
